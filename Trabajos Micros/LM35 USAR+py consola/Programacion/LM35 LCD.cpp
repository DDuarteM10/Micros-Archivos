...
void Confi(void){
	...
	GPIOA -> MODER |= 0xA3;//Funcion analoga A0 y  A2Tx y A3Rx  alterno
	GPIOB -> MODER |= 0x1;//Funcion analoga A0
	...
	USART2->CR1 = 0x012C; // Activar TX, RX
  USART2->CR1 |= 0x2000; // Enable USART
	GPIOA->AFR[0] |= 0x7700; // select AF7 (USART2) for PA2, PA3 PIN 2 Y 3
}
void USAR_SEN(long x){
	USART2 -> DR=x;
...
int main(void){
	...
	while(1){
		...
		GPIOB -> ODR =0x1;//Bandera
		contador2=0;
		while(variable[contador2]!=0){
			for(long x=0;x<1000000;x++){__NOP();}
